$(function () {
    ...
    $(".btn.edit").on("click", function () {
        ...
        if (list.length === 0) {
            openAlert("Please select a record to edit.");
            return false;
        }
        else if (list.length > 1) {
            openAlert("Please just select a record to edit.");
            return false;
        } else {
            var id = list[0];
    ...
    $(".btn.delete").on("click", function () {
        ...
        if (arr.length === 0) {
            openAlert("Please at least select a record to delete");
            return false;
        } else {
            var url = $(this).attr("rel");
            openConfirm("Are you sure that you want to delete?", function () {
                var toUrl = "";
                if (url.indexOf("?") >= 0) {
...
function openConfirm(message, callback, title) {
    ...
    $("#dlgmsg").attr("title", "Confirmation");
    $("#dlgmsg-text").html(message);
    $("#dlgmsg").dialog({
        position: {
            my: "center top",
